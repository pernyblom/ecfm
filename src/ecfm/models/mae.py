...
class EventMAE(nn.Module):
    """Region-token masked autoencoder.

    Masked patch content is replaced before encoding, while region metadata and
    plane identity remain visible. This is intentionally a masked-token
    reconstruction model rather than a visible-tokens-only MAE encoder.
    """
    ...
    def forward(
        ...
        decoded = self.decoder(decoder_in, src_key_padding_mask=padding_mask)
        ...
        if padding_mask is not None:
            pred_patch = pred_patch.masked_fill(
                padding_mask[:, :, None, None, None], 0.0
            )
            pred_count = pred_count.masked_fill(padding_mask.unsqueeze(-1), 0.0)
