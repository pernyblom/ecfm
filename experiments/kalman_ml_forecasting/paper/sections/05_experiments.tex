...
The image-conditioned experiments use one image representation per run together with fixed cutouts, relative box-history features, frame-centered filtered center positions, optimized Kalman state features, and a ResNet-18 encoder. We do not use Kalman covariance features in the main results. We report single-input comparisons selected to separate event-frame, CSTR, time-space projection, RGB, and filter-state-only effects.
...
The second stage evaluates event-conditioned residuals on the standard split. Early experiments suggest that event-derived inputs improve box metrics beyond the optimized Kalman baseline on this split. Dataset-native event frames have been the strongest single image input in the raw split among the tested formats. The final table therefore focuses on individual image inputs.
...
\begin{table}[t]
  ...
  \begin{tabular}{lcccccc}
    ...
    Residual & XT & fixed & \todo{} & \todo{} & \todo{} & \todo{} \\
    Residual & YT & fixed & \todo{} & \todo{} & \todo{} & \todo{} \\
    Residual & RGB & fixed & \todo{} & \todo{} & \todo{} & \todo{} \\
    \bottomrule
  \end{tabular}
  }
\end{table}
...
\begin{table}[t]
  ...
  \begin{tabular}{lcccccc}
    ...
    Residual & CSTR3 & 0.5 & \todo{} & \todo{} & \todo{} & \todo{} \\
    Residual & best single image input & 0.5 & \todo{} & \todo{} & \todo{} & \todo{} \\
    \bottomrule
  \end{tabular}
  }
\end{table}
...
The following ablations are useful extensions: cutout size, temporal bin count, temporal crop length for XT/YT, smaller CNN backbones, covariance features, and RGB contribution. We report them as limitations or future work unless included in the final result tables.
