...
def _infer_frame_size(
    ...
    for items in tracks.values():
        for _, x, y, w, h in items:
            max_x = max(max_x, x + w)
            max_y = max(max_y, y + h)
    return int(np.ceil(max_x)), int(np.ceil(max_y))
...
class LeWorldModelTrackDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        max_frame_gap_s: Optional[float] = 0.05,
        ...
        self.frame_size_override = tuple(frame_size) if frame_size else None
    ...
    def _cache_path(self) -> Optional[Path]:
        ...
        return self.cache_dir / f"lewm_samples_{self._cache_key()}.pkl"
