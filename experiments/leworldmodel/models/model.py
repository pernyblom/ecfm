...
class LeWorldModel(nn.Module):
    def __init__(self, cfg: Dict) -> None:
        ...
        data_cfg = cfg["data"]
        model_cfg = cfg["model"]
        self.context_steps = int(data_cfg["context_steps"])
        self.future_steps = int(data_cfg["future_steps"])
        ...
        self.predictor = build_predictor(model_cfg, self.latent_dim, self.context_steps)
    ...
    def rollout(self, context_latents: torch.Tensor) -> torch.Tensor:
        ...
        for _ in range(self.future_steps):
            next_latent = self.predictor(history[:, -self.context_steps :])
    ...
    def teacher_force_predictions(self, z_seq: torch.Tensor) -> torch.Tensor:
        preds = []
        for idx in range(self.future_steps):
            history = z_seq[:, idx : idx + self.context_steps]
    ...
    def forward(
        ...
        z_context = z_seq[:, : self.context_steps]
        z_future = z_seq[:, self.context_steps :]
        ...
        if self.forecast_head is not None and past_boxes is not None:
            out["pred_future_boxes"] = self.forecast_head(pred_future_rollout, past_boxes)
        return out
